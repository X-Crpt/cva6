    ...
    while(1){
        ...
        for (uint32_t i = 0; i < num_traces; i++) {
            ...
            *trigger = 1 << TRIGGER_CTRL_START; //Putting high the trigger
            ...
            // Since AES works on 128-bit blocks (which is 16 bytes), ciphertext is expected to be 16 bytes long.
            // You need to safely cast the uint8_t* to uint32_t* before passing it to AES_Cipher.
